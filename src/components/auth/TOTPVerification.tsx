...
interface TOTPRegistrationData {
  secretKey: string;
  qrCodeUrl: string;
  qrCodeImage: string;
}
...
export const TOTPSetup: React.FC<TOTPSetupProps> = ({ onSuccess, onCancel }) => {
  ...
  const [setupData, setSetupData] = useState<TOTPRegistrationData | null>(null);
  ...
  const initializeTOTPSetup = async () => {
    try {
      ...
      // const response = await mfaSettingServices.initiateTOTPSetup();
      // if (response.success) {
      ...
      setTimeout(() => {
        const mockData: TOTPRegistrationData = {
          secretKey: "PVLH6PXPOKTKWI6HB5OYQHDG3DL5JAVN",
          qrCodeUrl: "otpauth://totp/nguyenvana1@localhost?secret=PVLH6PXPOKTKWI6HB5OYQHDG3DL5JAVN&issuer=localhost",
          qrCodeImage: "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=otpauth://totp/nguyenvana1@localhost?secret=PVLH6PXPOKTKWI6HB5OYQHDG3DL5JAVN%26issuer=localhost"
        };
        setSetupData(mockData);
        setStep('setup'); // Đảm bảo chuyển sang bước setup
        setIsLoading(false);
      }, 1500);
  ...
  const handleVerifyTOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // const response = await mfaSettingServices.verifyAndEnableTOTP({
